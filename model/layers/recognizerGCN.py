...
from model.builder import RECOGNIZERS
from model.layers.recognizer import BaseRecognizer
...
class RecognizerGCN(BaseRecognizer):
    ...
    def forward_train(self, keypoint, label):
        ...
        losses = dict()
        x = self.extract_feat(keypoint) # 用backbone(gcn_vivit) extract input1024->output2048
    ...
    def forward_test(self, keypoint):
        """Defines the computation performed at every call when evaluation and
        testing."""
        assert self.with_cls_head or self.feat_ext
        bs, nc = keypoint.shape[:2] # bs=, nc=num_classes
        ...
        cls_score = self.cls_head(x)
        cls_score = cls_score.reshape(bs, nc, cls_score.shape[-1]) # bs,num_calsses, 
        # harmless patch
        ...
        # print(f'cls_score.data: {cls_score.data}')
        # print(f'cls_score.data.cpu().numpy(): {cls_score.data.cpu().numpy()}')
        # print(f'cls_score.data.shape: {cls_score.data.shape}')
